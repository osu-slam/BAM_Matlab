...
outputLatency = 110; % in milliseconds
...
p.sampleRate = 48000;  % sample rate for [melody, rhythm, pulse ]
...
for t=1:3
    ...
    audio_r_prac{t} = audio_tmp';
    
    stimfile = [dir_prac '/' char(pcond_m(t,3))];
    [audio_tmp,~] = audioread(stimfile);
    audio_m_prac{t} = audio_tmp';
...
InitializePsychSound(1);
pahandle = PsychPortAudio('Open', 1, [], 1, srate, 2);
ph44100 = PsychPortAudio('Open', 1, [], 1, 44100, 2);  % some stimuli have 44100 hz
...
while ~quitit
    ...
    % fill buffer
    PsychPortAudio('FillBuffer', ph44100, audio_p_prac{2});
    PsychPortAudio('Start', ph44100);
...
for t=1:nstim_p
    ...
    if is44100=="44100"
        ...
        PsychPortAudio('FillBuffer', ph44100, audio_pulse{t});
    else
